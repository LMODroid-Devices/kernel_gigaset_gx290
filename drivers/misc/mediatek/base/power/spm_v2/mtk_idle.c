 ...
 * GPT need to modify mtk_idle.c and mtk_spm_mcdi.c
 */
...
#define idle_emerg(fmt, args...)	pr_debug(IDLE_TAG fmt, ##args)
#define idle_alert(fmt, args...)	pr_debug(IDLE_TAG fmt, ##args)
...
#elif (MCDI_DVT_IPI)
/* localtimer_set_next_event(130000000); */
/* pr_debug("delay local timer next event"); */
#endif
#endif
#endif
}
...
#else
	idle_warn_log(
	  "DP:timer_left=%d, timer_left2=%d, delta=%d, timeout val=%d\n",
				dpidle_timer_left,
				dipidle_timer_left2,
...
#else
		idle_ver(
		 "SO3:timer_left=%d, timer_left2=%d, delta=%d,timeout val=%d\n",
			soidle3_timer_left, soidle3_timer_left2,
...
#else
		idle_ver(
		  "SO:timer_left=%d, timer_left2=%d, delta=%d,timeout val=%d\n",
			soidle_timer_left, soidle_timer_left2,
...
static int mcdi_cpu_notify(struct notifier_block *self,
			   unsigned long action, void *hcpu)
{
	...
	/**
	 * removed, because in kernel 4.9, there is no
	 * CPU_DYING and CPU_DYING_FROZEN
	case CPU_DYING:
			       ...
			       size_t count, loff_t *f_pos)
{
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
			      ...
			      i, soidle3_cnt[i], i, soidle_cnt[i]);
		p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
			      "mcidle_cnt[%d]=%lu, slidle_cnt[%d]=%lu, rgidle_cnt[%d]=%lu\n",
			      i, mcidle_cnt[i], i, slidle_cnt[i], i,
			      rgidle_cnt[i]);
	}
	...
	for (i = 0; i < NR_TYPES; i++)
		p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
			      "%s_switch=%d, ", idle_name[i], idle_switch[i]);

	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf), "\n");
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "idle_ratio_en = %u\n", mtk_idle_get_ratio_status());
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "twam_handler:%s (clk:%s)\n",
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "\n********** mcidle command help **********\n");
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "mcidlehelp cat/sys/kernel/debug/cpuidle/mcidle_state\n");
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "dpidle_dump_log = %u\n", dpidle_dump_log);
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "(0: None, 1: Reduced, 2: Full\n");

	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "\n*********** dpidle command help  ************\n");
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "dpidlehelpcat /sys/kernel/debug/cpuidle/dpidle_state\n");
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
...
static ssize_t soidle3_state_read(struct file *filp, char __user *userbuf,
				  size_t count, loff_t *f_pos)
{
	int len = 0;
	char *p = dbg_buf;
				 ...
				 loff_t *f_pos)
{
	...
	for (i = 0; i < NR_GRPS; i++) {
		p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
			     ...
			     i, cg_grp_get_name(i),
			      slidle_condition_mask[i], cg_grp_get_name(i),
			     slidle_block_mask[i]);
	}
...
static ssize_t reg_dump_read(struct file *filp, char __user *userbuf,
			     size_t count, loff_t *f_pos)
{
	int len = 0;
	char *p = dbg_buf;
	...
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_VEN_PWR_CON = 0x%08x\n", idle_readl(SPM_VEN_PWR_CON));
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_DIS_PWR_CON = 0x%08x\n", idle_readl(SPM_DIS_PWR_CON));
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_AUDIO_PWR_CON = 0x%08x\n", idle_readl(SPM_AUDIO_PWR_CON));
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_MD1_PWR_CON = 0x%08x\n", idle_readl(SPM_MD1_PWR_CON));
/*	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
 *"SPM_MD2_PWR_CON = 0x%08x\n", idle_readl(SPM_MD2_PWR_CON));
 */
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_C2K_PWR_CON = 0x%08x\n", idle_readl(SPM_C2K_PWR_CON));
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_CONN_PWR_CON = 0x%08x\n", idle_readl(SPM_CONN_PWR_CON));
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf),
		      "SPM_MDSYS_INTF_INFRA_PWR_CON = 0x%08x\n",
...
static int mtk_cpuidle_debugfs_init(void)
{
	...
	debugfs_create_file("idle_state", 0644, root_entry,
			    NULL, &idle_state_fops);
	debugfs_create_file("dpidle_state", 0644, root_entry,
			    NULL, &dpidle_state_fops);
	debugfs_create_file("soidle3_state", 0644, root_entry,
			    NULL, &soidle3_state_fops);
	debugfs_create_file("soidle_state", 0644, root_entry,
			    NULL, &soidle_state_fops);
	debugfs_create_file("mcidle_state", 0644, root_entry,
			    NULL, &mcidle_state_fops);
	debugfs_create_file("slidle_state", 0644, root_entry,
			    NULL, &slidle_state_fops);
	debugfs_create_file("reg_dump", 0644, root_entry,
			    NULL, &reg_dump_fops);
...
{
	mtk_idle_twam_init();
	mtk_idle_block_setting(IDLE_TYPE_DP, dpidle_cnt, dpidle_block_cnt,
			       dpidle_block_mask);
	mtk_idle_block_setting(IDLE_TYPE_SO3, soidle3_cnt, soidle3_block_cnt,
			       soidle3_block_mask);
	mtk_idle_block_setting(IDLE_TYPE_SO, soidle_cnt, soidle_block_cnt,
			       soidle_block_mask);
	mtk_idle_block_setting(IDLE_TYPE_SL, slidle_cnt, slidle_block_cnt,
			       slidle_block_mask);
